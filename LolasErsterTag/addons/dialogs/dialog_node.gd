...
export(Resource) var dialog_resource = load("res://addons/dialogs/Resources/DialogResources/DefaultDialogResource.tres")
...
#						button.connect("pressed", self, "change_position", [button, 0])
		...
		{'action'}:
			...
			if event['action'] == 'focusout_portraits':
				...
				load_dialog(true)
		{'scene'}:
			var audioNode = get_parent().get_node("AudioStreamPlayer")
			Global.audioPosition = audioNode.get_playback_position()
			print(Global.audioPosition)
